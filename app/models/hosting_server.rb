class HostingServer < ApplicationRecord
  ...
  def to_chef_json
    hosting_server = serializable_hash
    hosting_server.keep_if do |key, value|
      %w(
    ...
    hosting_server['os'] = OSES[os_id].first.downcase
    hosting_server['mysql_distrib'] = MYSQL_DISTRIBS[mysql_distrib_id].first.downcase
    hosting_server['delivery_method'] = MAIL_DELIVERY_METHODS[mail_delivery_method_id].first
    hosting_server['smtp_settings'] = {}
    smtp_settings.each do |ss|
      hosting_server['smtp_settings'][ss.name] = ss.value
    end
    hosting_server['apache_variations'] = {}
    apache_variations.each do |av|
      hosting_server['apache_variations'][av.name] = {
          ip: av.ip,
